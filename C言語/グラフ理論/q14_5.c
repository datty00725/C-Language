//、路線図データを表すグラフの直径を求める関数diameter を定義
#include <stdio.h>
#include <stdlib.h>
char buf[256];
...
int rad(struct node *adjlist[], int n, int dore, int soukyori)
{
    int kyorikyori = 1;
    for (int i = 0; i < n; i++)
    {
        int ikisaki[n], kyoori[n];
        for (int j = 0; j < n; j++)
        {
            ikisaki[j] = 1;
            kyoori[j] = 0;
        ...
        for (int j = 0; j < n; j++)
        {
            if (ikisaki[j] == 1)
            {
                kyorikyori += rad(adjlist, n, ikisaki[j], )
            }
        }
    }
}
